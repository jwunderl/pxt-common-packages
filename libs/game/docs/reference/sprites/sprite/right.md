...
Send a sprite moving from the left side of the screen to the right. In an ``||game:on game update||`` loop, check to see if the sprite touched the right side of the screen. If so, reset the sprite back to the left side of the screen.
